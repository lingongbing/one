<template>
	<form class="form-horizontal" v-on:submit.prevent="onSubmit">
		...
		<div class="form-group">
			<div class="col-sm-12">
				<button type="submit" class="btn btn-default">更新商品</button>
			</div>
		</div>
	</form>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			getGood:function (good) {
				window.axios.get('integral-goods/'+good).then(response => {
					this.name = response.data.name;
					this.integral = response.data.integral;
					this.editor.setValue(response.data.introduction);
				})
			},
			onSubmit:function () {
				this.$validator.validateAll().then(result => {
					if (result) {
						window.axios.patch('integral-goods/'+this.good,{
							name:this.name,
							integral:this.integral,
							introduction:this.editor.getValue()
						}).then(response => {
							this.message = this.name + '更新成功';
						}).catch(error => {
